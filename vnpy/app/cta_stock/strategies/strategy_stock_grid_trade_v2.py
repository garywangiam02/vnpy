...
from vnpy.component.cta_utility import check_bi_not_rt, ChanSignals

########################################################################
class StrategyStockGridTradeV2(CtaStockTemplate):
    """股票网格交易策略
        1、筛选股票
            1）通过基本面选出来的个股，N只票，N<=10
    ...
    def on_bar_k(self, bar: BarData):
        ...
        # 持有多单时，才进行检查
        if pos.volume > 0:
            self.tns_xt_logic(bar.vt_symbol)

    def tns_xt_logic(self, vt_symbol):
        """
        事务根据形态信号，提前结束多单,或者更新止损价
        :param vt_symbol:
        :return:
        """
        ...
        if check_bi_not_rt(kline, direction=Direction.LONG):
            ...
            for n in [5,7]:
                ...
                if xt_signals and len(xt_signals) > 0:
                    ...
                    if xt_signal['signal'] == ChanSignals.SI0.value:
                        has_SI0 = True
                        break

        # 处理三卖信号
        if has_SI0:
            grid_plan = self.policy.grids[vt_symbol]
            open_prices = grid_plan['open_prices']
            for grid in self.gt.dn_grids:
                if grid.vt_symbol == vt_symbol:
                    # 未设置止损价或止损价低于当前笔低点时，才进行
                    if grid.stop_price == 0 or grid.stop_price < kline.cur_bi.low:
                        # 更新止损价为当前笔的low
                        self.write_log(f'{vt_symbol}[{cn_name}]满足形态3卖，更新止损价:{grid.stop_price}=>{float(kline.cur_bi.low)}')
                        grid.stop_price = float(kline.cur_bi.low)

                        dist_record = OrderedDict()
                        dist_record['datetime'] = self.cur_datetime
                        dist_record['symbol'] = vt_symbol
                        dist_record['volume'] = grid.volume
                        dist_record['price'] = float(kline.cur_bi.low)
                        dist_record['operation'] = '3 sell'
                        self.save_dist(dist_record)

                        # 剔除当前以上所有网格开空计划价格
                        open_prices = [p for p in open_prices if p < grid.open_price]
                        grid_plan['open_prices'] = open_prices
                        self.write_log(f'{vt_symbol}[{cn_name}]满足形态3卖,更新网格计划=>{open_prices}')
                        self.policy.save()
    ...
    def tns_update_grids(self, vt_symbol):
        ...
        # 初始化/价格高于最高价,或没有价格
        if len(grid_info) == 0 or cur_price > grid_info.get('high_price', cur_price) or len(grid_info.get('open_prices',[]))==0:
        ...
        if len(grids) == 0:
            ...
            # 如果K线得线段为空白，则不开仓
            if len(kline.duan_list) == 0 and len(kline.bi_list) == 0:
                self.write_log(f'{vt_symbol}[{cn_name}]的K线线段、分笔都未生成')
                return
            ...
            if len(kline.duan_list) > 0:
                ...
                if cur_duan.direction == 1:
                    self.write_log(f'{vt_symbol}[{cn_name}]的K线线段为向上')
                    return
            else:
                # 如果改分笔为上涨分笔，不做买入
                cur_bi = kline.bi_list[-1]
                if cur_bi.direction == 1:
                    self.write_log(f'{vt_symbol}[{cn_name}]的K线分笔为向上')
                    return
                ...
                if len(all_prices) > len(unopen_prices):
                    if len(unopen_prices) > 0:
                        ...
                        if max_price:
                            ...
                            self.write_log(f'{vt_symbol}[{cn_name}]调整所有待开仓价格:{all_prices}=> {unopen_prices}, 最高价:=>{high_price}')
                            extend_open_prices = [round(low_price - (i + 1) * grid_height, 2) for i in range(self.grid_lots-len(unopen_prices))]
            ...
            self.tns_add_long_grid(
                ...
                close_price=open_price + grid_height,
