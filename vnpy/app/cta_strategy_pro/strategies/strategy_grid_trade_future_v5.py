...
########################################################################
class StrategyGridTradeFutureV5(CtaProFutureTemplate):
    """期货网格交易策略
    ...
    def __init__(self, cta_engine,
        ...
        self.policy = Future_Grid_Trade_PolicyV5(self)
    ...
    def update_grids(self, vt_symbol):
        ...
        if self.active_long_grid:
            ...
            # 当前价格距离最高价很远，并存在做多线段时，看是否可以补充下做多计划
                ...
                if self.kline_x.cur_duan and self.kline_x.cur_duan.direction == 1 \
                        ...
                        and grid_plan['high_price'] - 2 * grid_height > self.kline_x.cur_duan.high > max_open_price + grid_height:

                    # 新增加一个
                    new_open_price = round_to(max_open_price + grid_height, price_tick)
                    # 添加到计划中
                    grid_plan['long_prices'].update({new_open_price:self.grid_repeats})
                    self.write_log(f'新增做多计划：{new_open_price}')
                    d = {
                        "datetime": self.cur_datetime,
                        "price": new_open_price,
                        "operation": 'append long',
                        "signal": f'plan',
                        "stop_price": 0
                    }
                    self.save_dist(d)
        ...
        if self.active_short_grid:
            ...
            #  当前价格距离最低价很远，并存在做空线段时，看是否可以补充下做空计划
                ...
                if self.kline_x.cur_duan and self.kline_x.cur_duan.direction == -1 \
                        and self.kline_x.cur_duan.end == self.kline_x.cur_bi.end \
                        and grid_plan['low_price'] + 2 * grid_height < self.kline_x.cur_duan.low < min_open_price - grid_height:

                    # 新建一个价格
                    new_open_price = round_to(min_open_price - grid_height, price_tick)
                    # 添加到计划中
                    grid_plan['short_prices'].update({new_open_price:self.grid_repeats})
                    self.write_log(f'新增做空计划：{new_open_price}')
                    d = {
                        "datetime": self.cur_datetime,
                        "price": new_open_price,
                        "operation": 'append short',
                        "signal": f'plan',
                        "stop_price": 0
                    }
                    self.save_dist(d)
            ...
            if self.is_chanlun_fit(direction=Direction.LONG, vt_symbol=vt_symbol):
                ...
                if last_opened_price:
                    ...
                    if remove_long_prices:
                        self.remove_plan_open_price(vt_symbol= vt_symbol,
                                                    direction=Direction.LONG,
                                                    open_prices=remove_long_prices)

                # 如果仍然有做多计划价格
                if len(plan_long_prices) > 0:
                    # 找出上一次开多的分笔开始时间
                    last_long_bi_start = grid_plan.get('last_long_bi_start', None)
                    # 最低的开仓价
                    open_price = plan_long_prices[0]
                    # 如果有多个计划开仓价（缠论阻挡了这些价格的开仓）
                    volume_rate = len(plan_long_prices)

                    # 当前笔的开始时间，与上一开多的所在笔开始开始不一致，可以开新多
                    # 或者， 处于相同笔内，一定要跳一个格子才加多
                    if last_long_bi_start != self.kline_x.cur_bi.start or (
                            last_long_bi_start == self.kline_x.cur_bi.start and volume_rate > 1):
                        # 获取高度
                        ...
                        if len(grids) == 0:
                            ...
                            if self.tns_add_long_grid(
                                ...
                                grid_plan.update({'last_long_bi_start': self.kline_x.cur_bi.start})
                                self.policy.grids.update({vt_symbol: grid_plan})
                                self.policy.save()
                ...
                if last_opened_price:
                    ...
                    if remove_short_prices:
                        self.remove_plan_open_price(vt_symbol=vt_symbol,
                                                    ...
                                                    open_prices=remove_short_prices)

                if len(plan_short_prices) > 0:
                    # 找出上一次开空的分笔开始时间
                    last_short_bi_start = grid_plan.get('last_short_bi_start', None)
                    # 找出最大的开空价格
                    open_price = plan_short_prices[-1]
                    # 开仓格子数量（如果缠论阻挡了部分开空价，那么plan_short_prices就可能存在几个）
                    volume_rate = len(plan_short_prices)

                    # 当前笔的开始时间，与上一开空的所在笔开始开始不一致，可以开新空
                    # 或者， 处于相同笔内，一定要跳一个格子才加空
                    if last_short_bi_start != self.kline_x.cur_bi.start or (
                            last_short_bi_start == self.kline_x.cur_bi.start and volume_rate > 1):
                        ...
                        if len(grids) == 0:
                            ...
                            if self.tns_add_short_grid(
                                ...
                                # 记录当次开空的所在分笔位置
                                grid_plan.update({'last_short_bi_start': self.kline_x.cur_bi.start})
                                self.policy.grids.update({vt_symbol: grid_plan})
                                self.policy.save()
    ...
    def is_chanlun_fit(self, direction, vt_symbol):
        ...
        # 至少包含两个线段，才开始
        if self.kline_x.pre_duan is None:
            return False

        if self.kline_x.cur_duan:
            if len(self.kline_x.cur_duan.bi_list) == 1:
                return False
            if self.kline_x.cur_duan.end != self.kline_x.cur_bi.end:
                return False
            ...
            else:  # direction == Direction.SHORT
                # 如果线段为下跌线段，不做卖出
                if self.kline_x.cur_duan.direction == -1:
    ...
    def is_chanlun_break_duan(self, direction):
        """
        缠论是否打破原有的线段
        :param direction: 线段得方向
        :return:
        """
        # 存在线段、且线段内分笔至少存在3笔
        if self.kline_x.cur_duan is None or len(self.kline_x.bi_list) < 3 or self.kline_x.cur_duan.end == self.kline_x.cur_bi.end:
            return False
        ...
        # 线段外的分笔
        extra_bi_list = [bi for bi in self.kline_x.bi_list[-10:] if bi.start >= self.kline_x.cur_duan.end]
        # 线段外得首个分笔
        extra_bi = extra_bi_list[0]

        if direction == Direction.LONG:
            second_high = max([bi.high for bi in self.kline_x.cur_duan.bi_list])
            if extra_bi.low < second_high:
                return True
        else:
            second_low = min([bi.low for bi in self.kline_x.cur_duan.bi_list])
            if extra_bi.high > second_low:
                return True
        return False
    ...
    def get_last_opened_price(self, direction, vt_symbol):
        ...
        if opened_prices:
            return max(opened_prices), len(opened_prices)
        else:
            return None, 0

    def tns_add_long_grid(self, vt_symbol, open_price, close_price, volume_rate=1, bi_start = None):
        """
        事务添加做多网格
        :param :
        :return:
        ...
        if bi_start:
            grid.snapshot.update({'bi_start': self.kline_x.cur_bi.start})
    ...
    def tns_add_short_grid(self, vt_symbol, open_price, close_price, volume_rate=1,bi_start=None):
        """
        事务添加做空网格
        :param :
        :return:
    ...
    def tns_check_grids(self):
        ...
        _,long_grid_num = self.get_last_opened_price(direction=Direction.LONG,vt_symbol=self.vt_symbol)
        _,short_grid_num = self.get_last_opened_price(direction=Direction.SHORT, vt_symbol=self.vt_symbol)
            ...
            if grid.open_status:
                ...
                if self.last_long_close_price and self.last_long_close_price < grid.close_price:
                    ...
                    d = {
                        "datetime": self.cur_datetime,
                        "price": cur_price,
                        "operation": 'change close3',
                        "signal": f'{grid.close_price} => {grid.open_price + self.price_tick}',
                        "stop_price": 0
                    }
                    self.save_dist(d)
                    grid.close_price = self.last_long_close_price
                    self.remove_plan_open_price(vt_symbol=grid.vt_symbol,
                                                direction=Direction.LONG,
                                                open_prices=[grid.open_price])

                # 多单主动止盈价格修正：有且仅有一个格，当前价格在开平仓价格之间
                if long_grid_num == 1 and grid.open_price < cur_price < grid.close_price:
                    # 当前分型
                    cur_fx = self.kline_x.fenxing_list[-1]

                    if grid.stop_price == 0 and self.kline_x.cur_duan.direction == 1 \
                            and grid.close_price > self.kline_x.cur_bi.high > grid.open_price + (grid.close_price - grid.open_price)/2\
                            and cur_fx.direction ==1 and not cur_fx.is_rt:
                        stop_price = grid.open_price + (grid.close_price - grid.open_price)/3
                        self.write_log(f'主动设置保护止损价 0 => {stop_price}')
                        grid.stop_price = stop_price
                        d = {
                            "datetime": self.cur_datetime,
                            "price": cur_price,
                            "operation": 'change stop',
                            "signal": f'0 => {grid.stop_price}',
                            "stop_price": stop_price
                        }
                        self.save_dist(d)

                    # 当前下跌线段已经被破坏
                    if self.is_chanlun_break_duan(direction=Direction.SHORT):
                        last_long_bi_start = self.policy.grids.get(grid.vt_symbol,{}).get('last_long_bi_start',None)
                        if last_long_bi_start and last_long_bi_start < self.kline_x.cur_duan.end:

                            # 主动止盈1: 线段破坏+分笔反包
                            if self.kline_x.cur_bi.direction == 1 and self.kline_x.cur_bi.start == self.kline_x.cur_duan.end:
                                # 当前顶分型已经成立
                                if cur_fx.direction == 1 and not cur_fx.is_rt and self.kline_x.cur_bi.high >= self.kline_x.cur_duan.bi_list[-1].high:
                                    self.write_log(
                                        f'主动修改多单止盈价:{grid.close_price} => {grid.open_price + self.price_tick}')
                                    d = {
                                        "datetime": self.cur_datetime,
                                        "price": cur_price,
                                        "operation": 'change close1',
                                        "signal": f'{grid.close_price} => {grid.open_price + self.price_tick}',
                                        "stop_price": 0
                                    }
                                    self.save_dist(d)
                                    self.remove_plan_open_price(vt_symbol=grid.vt_symbol,
                                                                direction=Direction.LONG,
                                                                open_prices=[grid.open_price])
                                    grid.close_price = grid.open_price + self.price_tick

                            # 主动止盈2：线段初步破坏，可能继续走低，当前分笔向上，分笔开始不等于线段结束
                            if self.kline_x.cur_bi.direction == 1 and self.kline_x.cur_bi.start != self.kline_x.cur_duan.end:
                                # 当前顶分型已经成立
                                if cur_fx.direction == 1 and not cur_fx.is_rt:
                                    self.write_log(f'主动修改多单止盈价:{grid.close_price} => {grid.open_price+self.price_tick}')
                                    d = {
                                        "datetime": self.cur_datetime,
                                        "price": cur_price,
                                        "operation": 'change close2',
                                        "signal": f'{grid.close_price} => {grid.open_price + self.price_tick}',
                                        "stop_price": 0
                                    }
                                    self.save_dist(d)
                ...
                if grid.stop_price and grid.stop_price > cur_price:
                    self.write_log(f'多单满足止损条件')
                    if self.grid_sell(grid):
                        grid.close_status = True
                        grid.order_status = True
                        continue
        ...
        remove_gids = []
        for grid in self.gt.up_grids:   # 逐一检查做空网格
            ...
            if grid.open_status:
                ...
                if self.last_short_close_price and self.last_short_close_price > grid.close_price:
                    ...
                    d = {
                        ...
                        "price": cur_price,
                        "operation": 'change close3',
                        "signal": f'{grid.close_price} => {self.last_short_close_price}',
                        "stop_price": 0
                    }
                    self.save_dist(d)
                ...
                # 空单主动止盈价格修正：有且仅有一个格，当前价格在开平仓价格之间
                if short_grid_num == 1 and grid.open_price > cur_price > grid.close_price:
                    cur_fx = self.kline_x.fenxing_list[-1]

                    if grid.stop_price == 0 and self.kline_x.cur_duan.direction == -1 \
                            and grid.close_price < self.kline_x.cur_bi.low < grid.open_price - (grid.close_price - grid.open_price)/2\
                            and cur_fx.direction ==-1 and not cur_fx.is_rt:
                        stop_price = grid.open_price - (grid.close_price - grid.open_price)/3
                        self.write_log(f'主动设置保护止损价 0 => {stop_price}')
                        grid.stop_price = stop_price
                    ...
                    # 当前上涨线段已经被破坏
                    if self.is_chanlun_break_duan(direction=Direction.LONG): # 当前分型
                        last_short_bi_start = self.policy.grids.get(grid.vt_symbol, {}).get('last_short_bi_start', None)
                        if last_short_bi_start and last_short_bi_start < self.kline_x.cur_duan.end:

                            # 主动止盈1: 线段破坏+分笔反包
                            if self.kline_x.cur_bi.direction == -1 and self.kline_x.cur_bi.start == self.kline_x.cur_duan.end:
                                # 当前低分型已经成立
                                if cur_fx.direction == -1 and not cur_fx.is_rt and self.kline_x.cur_bi.low <= \
                                        self.kline_x.cur_duan.bi_list[-1].low:
                                    self.write_log(
                                        f'主动修改空单止盈价:{grid.close_price} => {grid.open_price - self.price_tick}')
    ...
    def new_plan_open_price(self, vt_symbol, direction):
        pass

    def remove_plan_open_price(self,vt_symbol, direction, open_prices):
        """
        移除policy的价格计划中某个价格
        :param vt_symbol:
        :param direction:
        ...
        grid_plan = self.policy.grids.get(vt_symbol, {})
        if direction == Direction.LONG:
            plan_prices = grid_plan.get('long_prices',{})
            poped = False
            for open_price in open_prices:
                if open_price in plan_prices:
                    plan_prices.pop(open_price,None)
                    poped = True
                    self.write_log(f'做多价格计划内，移除{open_price}')
                    d = {
                        "datetime": self.cur_datetime,
                        "price": open_price,
                        "operation": 'remove long',
                        "signal": f'plan',
                        "stop_price": 0
                    }
                    self.save_dist(d)
            ...
            plan_prices = grid_plan.get('short_prices', {})
            poped = False
            for open_price in open_prices:
                if open_price in plan_prices:
                    plan_prices.pop(open_price, None)
                    poped = True
                    self.write_log(f'做空价格计划内，移除{open_price}')
                    d = {
                        "datetime": self.cur_datetime,
                        "price": open_price,
                        "operation": 'remove short',
                        "signal": f'plan',
                        "stop_price": 0
                    }
                    self.save_dist(d)
...
class Future_Grid_Trade_PolicyV5(CtaPolicy):
