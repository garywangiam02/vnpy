...
########################################################################
class StrategyMacdChannelGroup_v1_1(CtaFutureTemplate):
 ...
 v1:
    ...
    def is_macd_signal(self, kline, direction):
        ...
        if direction == Direction.LONG:
            # 趋势信号1，当前属于零轴上方macd的金叉，判断是否突破上一个金叉周期的最高位
            if cur_duan.direction == 1 \
                ...
                if cur_zs.end > cur_duan.start and cur_duan.high < cur_zs.high:
                    if (not open_price) or open_price > kline.cur_price:
                        return False, ''
                # 排除2：
                if tre_duan and pre_duan.height < tre_duan.height* 0.5 and pre_duan.height > cur_duan.height:
                        return False, ""
                d = {
            ...
            # 需要参照前一线段
            if cur_duan.direction == -1 and tre_duan and cur_duan.height < pre_duan.height * 0.618:
                # 反转信号03： 发生段内中枢进入分笔与离开分笔得背驰
