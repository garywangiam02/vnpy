...
########################################################################
class StrategyMacdChannelGroup_v1_7(CtaFutureTemplate):
    ...
    def is_macd_signal(self, kline, direction):
        ...
        if direction == Direction.LONG:
            ...
            if open_price and last_signal and duan_start:
                ...
                # macd seg段是多，持仓是空单，开仓价低于当前价,反手做多
                if 0 < cur_seg['macd_count'] <= 5 \
                        and last_signal == 'short' \
                        and signal_name in ['short_break'] \
                        and kline.cur_price > open_price \
                        and last_signal_time > pre_seg['start']:
                    d = {
                        "datetime": self.cur_datetime,
                        "price": kline.cur_price,
                        "operation": 'rev',
        ...
        if direction == Direction.SHORT:
            ...
            # 突破或底背离失败
            if open_price and last_signal and duan_start:
                # macd seg段是空，持仓是多单，开仓价高于当前价,反手做空
                if -5 <= cur_seg['macd_count'] < 0 \
