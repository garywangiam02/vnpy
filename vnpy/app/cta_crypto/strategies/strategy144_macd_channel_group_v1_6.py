...
########################################################################
class StrategyMacdChannelGroup_v1_6(CtaFutureTemplate):
 ...
 v1:
    ...
    def __init__(self, cta_engine,
        ...
        self.debug_dates = ['2020-01-18', '2021-02-06', '2021-02-07', '2021-02-22']
        self.write_log(f'{self.strategy_name}构造函数执行完成')
    ...
    def is_macd_signal(self, kline, direction):
        ...
        open_price = signal.get('open_price', kline.cur_price)
        duan_start = signal.get('duan_start', cur_duan.start)
        ...
        if direction == Direction.LONG:
            ...
            # or (duan_start == cur_duan.start and last_signal_time < cur_duan.bi_list[0].end))\

            if last_signal == 'short' \
                    and last_signal_time < cur_seg['start'] \
                    and open_price > kline.cur_price \
                    and (duan_start < cur_duan.start \
                         or (duan_start == cur_duan.start and open_price - kline.cur_price > 5 * kline.bi_height_ma())):
                if cur_duan.direction == -1 \
                        and cur_duan.low == cur_bi.low \
                        and (len(cur_duan.bi_list) >= 3 or cur_duan.height > 2 * kline.bi_height_ma()) \
            ...
            if open_price and last_signal and duan_start:
                ...
                if cur_duan.direction == 1 \
                        ...
                        and stop_price and stop_price < float(cur_duan.high) \
        ...
        if direction == Direction.SHORT:
            ...
            #  (duan_start == cur_duan.start and last_signal_time < cur_duan.bi_list[0].end) or \

            if last_signal == 'long' \
                    and last_signal_time < cur_seg['start'] \
                    and open_price < kline.cur_price \
                    and (duan_start < cur_duan.start or \
                         (duan_start == cur_duan.start and kline.cur_price - open_price > 5 * kline.bi_height_ma())):
                if cur_duan.direction == 1 \
            ...
            if open_price and last_signal and duan_start:
                ...
                if cur_duan.direction == -1 \
                        ...
                        and stop_price and stop_price > float(cur_duan.low) \
                        and cur_duan.start < duan_start < cur_duan.end:
                    d = {
                        "datetime": self.cur_datetime,
                        "price": kline.cur_price,
        ...
        for kline_name, kline in self.klines.items():
            if kline.cur_duan:
                if kline.cur_duan.direction == 1:
                    self.write_log(
                        f'{kline_name}: 上涨线段:{kline.cur_duan.start} [{kline.cur_duan.low}] =>{kline.cur_duan.end}[{kline.cur_duan.high}]')
                else:
                    self.write_log(
                        f'{kline_name}: 下跌线段:{kline.cur_duan.start} [{kline.cur_duan.high}] =>{kline.cur_duan.end}[{kline.cur_duan.low}]')

            if len(kline.bi_list) > 0:
                cur_bi = kline.bi_list[-1]
                if cur_bi.direction == 1:
                    self.write_log(f'{kline_name}: 上涨分笔:{cur_bi.start} [{cur_bi.low}] =>{cur_bi.end}[{cur_bi.high}]')
                else:
                    self.write_log(
                        f'{kline_name}: 下跌分笔:{cur_bi.start} [{cur_bi.high}] =>{cur_bi.end}[{cur_bi.low}]')

            if len(kline.macd_segment_list) > 0:
                cur_seg = kline.macd_segment_list[-1]
                self.write_log(f'{kline_name}: macd分段:{cur_seg}')
