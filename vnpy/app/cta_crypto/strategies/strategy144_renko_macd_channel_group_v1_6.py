...
########################################################################
class StrategyRenkoMacdChannelGroup_v1_6(CtaFutureTemplate):
 ...
 v1:
    ...
    def is_macd_signal(self, kline, direction):
        ...
        if direction == Direction.LONG:
            ...
            if self.cur_datetime.strftime('%Y-%m-%d %H') in ['2019-09-25 03']:
                a = 1
            if last_signal == 'short' \
                    and last_signal_time < cur_seg['start'] \
                    and open_price > kline.cur_price \
                    and (duan_start < cur_duan.start \
                         or (duan_start == cur_duan.start and open_price - kline.cur_price > 3 * kline.bi_height_ma())):
                if cur_duan.direction == -1 \
                    ...
                    last_signal_time = kline.cur_datetime.strftime('%Y-%m-%d %H:%M:%S.%f')
        ...
        if direction == Direction.SHORT:
            ...
            if last_signal == 'long' \
                    ...
                    and (duan_start < cur_duan.start or \
                         (duan_start == cur_duan.start and kline.cur_price - open_price > 3 * kline.bi_height_ma())):
                if cur_duan.direction == 1 \
        ...
        for kline_name, signal in signals.items():
            ...
            try:
                if len(str_last_signal_time) > 0:
                    last_signal_time = datetime.strptime(str_last_signal_time.split('.')[0], '%Y-%m-%d %H:%M:%S')
                else:
                    last_signal_time = None
            except Exception as ex:
                last_signal_time = None
