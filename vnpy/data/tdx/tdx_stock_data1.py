...
class TdxStockData(object):
    ...
    def select_best_ip(self, ip_list, proxy_ip="", proxy_port=0, exclude_ips=[]):
        ...
        for index, result in enumerate(results[0:10]):
            self.aviliable_ip_pool['ip_{}'.format(str(index))] = result
    ...
    def connect(self, is_reconnect: bool = False):
        ...
        try:
            if self.api is None or not self.connection_status:
                ...
                self.api = TdxHq_API(heartbeat=True, auto_retry=True, raise_exception=True, multithread=True)
                ...
                if len(self.best_ip) == 0:
                    if len(self.aviliable_ip_pool.keys()) < 1:
                ...
                if len(self.proxy_ip) > 0 and self.proxy_port > 0:
                    ...
                    # 使用pip install pytdx
                    # self.api.connect(ip=self.best_ip['ip'], port=self.best_ip['port'])
                    if len(self.aviliable_ip_pool.keys()) > 0:
                        self.aviliable_ip_key = random.choice(list(self.aviliable_ip_pool.keys()))
                        aviliable_ip = self.aviliable_ip_pool[self.aviliable_ip_key]
                    else:
                        aviliable_ip = self.best_ip

                    print("*" * 100)
                    self.write_log(f'aviliable_ip : {aviliable_ip}')
                    self.current_ip = aviliable_ip['ip']
                    self.process_id = '{}-{}'.format(os.getppid(), os.getpid())
                    self.api.connect(ip=aviliable_ip['ip'], port=aviliable_ip['port'])

                    self.write_log(f'创建tdx连接, : {aviliable_ip}')
            ...
            if cur_ip is not None and cur_ip not in self.exclude_ips:
                ...
                self.best_ip = {}
            if self.aviliable_ip_key != '':
                self.aviliable_ip_pool.pop(self.aviliable_ip_key)
                self.aviliable_ip_key = ''
            return
    ...
    def get_bars(self,
        ...
        if not self.api:
            self.connect()
        self.write_log(u'*******current_ip:{}'.format(self.current_ip))
        self.write_log(u'*******:process_id{}'.format(self.process_id))
        ret_bars = []
        if self.api is None:
            return False, []
