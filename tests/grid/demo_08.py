...
# 示例代码
# 从pytdx下载某一主力合约的周期K线，识别出线段的背驰信号点，标注在图上
...
class DemoStrategy(FakeStrategy):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 如果之前存在，移除
        if os.path.exists(demo_08_dist):
            self.write_log(f'移除{demo_08_dist}')
            os.remove(demo_08_dist)
    ...
    def on_bar(self, *args, **kwargs):
        ...
        if self.kline.cur_bi.end != self.last_check_bi:
            # 指定线段内分笔的形态分析
            signal = check_chan_xt(self.kline, self.kline.cur_duan.bi_list[-13:])

            if signal != ChanSignals.Other.value:
                self.last_check_bi = self.kline.cur_bi.end
                # 写入记录
                append_data(file_name=demo_08_dist,
...
ui.show(snapshot_file="",
        d=snapshot,  # 切片数据
        dist_file=demo_08_dist,  # 本地dist csv文件
