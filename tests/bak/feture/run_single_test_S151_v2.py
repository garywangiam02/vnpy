...
test_setting['name'] = 'S151_dual_ma_{}'.format(datetime.now().strftime('%m%d_%H%M'))

# 测试时间段, 从开始日期计算，过了init_days天，才全部激活交易
test_setting['start_date'] = '20210101'
test_setting['init_days'] = 10
test_setting['end_date'] = '20211230'

# 测试资金相关, 资金最大仓位， 期初资金
test_setting['percent_limit'] = 80
...
max_invest_rate = 0.2  # round(1 / len(selected_symbol),2)
...
for symbol, symbol_info in symbol_datas.items():
    ...
    underlying_symbol = symbol_info.get('underlying_symbol')
    if underlying_symbol not in ['RB']:  # selected_symbol: #
        continue

    # 这里设置为0，即不设置最大真实持仓；或者从上述配置中设置真实持仓
    max_invest_pos = 5  # selected_symbol[underlying_symbol]

    vt_symbol = symbol + '.' + exchange

    strategy_setting.update({
        f"s151_dual_ma_group_{symbol}": {
            "class_name": "Strategy151DualMaGroupV2",
