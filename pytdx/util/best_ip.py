...
def ping(ip, port=7709, type_='stock', proxy_ip='', proxy_port=0):
    api = TdxHq_API()
    apix = TdxExHq_API()
    __time1 = datetime.datetime.now()
    try:
        if type_ in ['stock']:
            with api.connect(ip=ip, port=port, time_out=0.7, proxy_ip=proxy_ip, proxy_port=proxy_port):
            ...
            with apix.connect(ip=ip, port=port, time_out=0.7,proxy_ip=proxy_ip, proxy_port=proxy_port):
...
def select_best_ip(_type='stock', proxy_ip="", proxy_port=0):
    ...
    data = [ping(ip=x['ip'], port=x['port'], type_=_type, proxy_ip=proxy_ip, proxy_port=proxy_port) for x in ip_list]
    results = []
