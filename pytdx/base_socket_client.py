    ...
    def wrapper(self, *args, **kw):
        ...
        try:
            ...
            if self.raise_exception:
                to_raise = TdxFunctionCallError("calling function {} error".format(func.__qualname__))
...
# 使用socks的替代原生socket,支持socks5端口
# socket.socket = socks.socksocket

class TrafficStatSocket(socks.socksocket):
    """
    实现支持流量统计的socket类
    """
...
class BaseSocketClient(object):
    ...
    def connect(self, ip='101.227.73.20', port=7709, time_out=CONNECT_TIMEOUT, bindport=None, bindip='0.0.0.0',
        ...
        if len(proxy_ip) > 0 and proxy_port > 0:
            log.debug(u"use socks5:{} {}".format(proxy_ip, proxy_port))
            socks.set_default_proxy(socks.SOCKS5, addr=proxy_ip,port=proxy_port)
        ...
        try:
            ...
            if self.raise_exception:
                raise TdxConnectionError("other errors{}".format(str(e)))
            return False
