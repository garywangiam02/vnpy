...
# python容器文件
python_path = '/home/trade/anaconda3/envs/vnpy2/bin/python'
...
def trade_off():
    ...
    if ACTIVE_NIGHT:
        ...
        # 周六2:30  ~ 周一:8:45
        weekend = (day_of_week == 6 and now >= a) or (day_of_week == 7) or (
            now.isoweekday() == 1 and now <= a)
        # 周一~周五，早上2：30~9：45， 下午15:30 ~20:45
        off = (1 <= day_of_week <= 5 and ((a <= now <= b) or (c <= now <= d))) or weekend
        return off
    else:
