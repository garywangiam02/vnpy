...
event_list = ['event_money_flow_1', 'event_money_flow_2']
...
def cal_money_flow_event_each_stock(code):
    ...
    # 筛选事件
    df['event_money_flow_1'] = None
    df['event_money_flow_2'] = None
    df.loc[df['factor'] > 0.4, 'event_money_flow_1'] = 1
    df.loc[df['factor'] > 0.45, 'event_money_flow_2'] = 1
    

    # 删除没有事件的日期
    df.dropna(subset=['event_money_flow_1', 'event_money_flow_2'], how='all', inplace=True, axis=0)

    # 输出
    df['事件日期'] = df['交易日期']  # 直接新建一列，补rename
    df = df[['股票代码', '股票名称', '事件日期', 'event_money_flow_1', 'event_money_flow_2']]
...
if __name__ == '__main__':
    ...
    for event in event_list:
        ...
        print(df)
        df.to_pickle('{}/data/event/{}.pkl'.format(root_path, event))
