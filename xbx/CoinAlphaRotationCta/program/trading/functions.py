...
def fetch_binance_swap_candle_data(exchange, symbol, run_time, limit=LIMIT):
    once_nums = min(1500,limit)
    start_time = datetime.datetime.now() - limit * datetime.timedelta(hours = 1)   
    # ===开始抓取数据
    df_list = []
    end_time_since = datetime.datetime.now()

    while True:
        # 获取数据 
        kline = robust(exchange.fapiPublic_get_klines,{'symbol': symbol, 'interval': '1h', "endTime":end_time_since,'limit': once_nums})
        # 整理数据
        kline = pd.DataFrame(kline, dtype=float)  # 将数据转换为dataframe
        # 合并数据
        df_list.append(kline)
        t = pd.to_datetime(kline.iloc[0][0], unit='ms')
        end_time_since = exchange.parse8601(str(t))
        # 判断是否挑出循环
        if t <= start_time or kline.shape[0] <= 1:
            break
        # 抓取间隔需要暂停0.2s，防止抓取过于频繁
        time.sleep(1)
    ...
    df.rename(columns={0: 'candle_begin_time', 1: 'open', 2: 'high',
                       3: 'low', 4: 'close', 5: 'volume',6:'close_time', 7: 'quote_volume',8: 'trade_num',
               9: 'taker_buy_base_asset_volume', 10: 'taker_buy_quote_asset_volume',11: 'ignore'}, inplace=True)  # 重命名
...
# 不超过1500根K线
# @robust
def fetch_binance_swap_candle_data_with_limited(exchange, symbol, run_time, limit=LIMIT):
    """
    ...
    for strategy in stratagy_list:
        ...
        for symbol in symbol_list:
            ...
            for factor_dict in factors:
                ...
                df = eval(f'signal_{factor}')(df, para)  # 计算信号
            ...
            for offset in range(int(hold_period[:-1])):
                ...
                # 截取指定周期的数据
                period_df = period_df[
                    (period_df['s_time'] <= run_time - datetime.timedelta(hours=int(hold_period[:-1]))) &
                    (period_df['s_time'] > run_time - 2 * datetime.timedelta(hours=int(hold_period[:-1])))
                ]
                # 合并数据
...
def next_run_time(time_interval, ahead_seconds=5, cheat_seconds=100):
    ...
    if cheat_seconds > 0.1:
        target_time = target_time - datetime.timedelta(seconds=cheat_seconds)
