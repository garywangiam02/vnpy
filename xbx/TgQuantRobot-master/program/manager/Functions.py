...
def fetch_binance_swap_candle_data(exchange, symbol, run_time, limit=1000):
    ...
    columns = ['symbol', 'candle_begin_time', 'open', 'high', 'low', 'close', 'volume', 'quote_volume', 'taker_buy_quote_asset_volume']
    df = df[columns]
...
def robust_order(actual_do,*args, **keyargs):
    tryTimes    = 5
    sleepTimes  = 10
    result = run_function_till_success_order(function=lambda: actual_do(*args, **keyargs), tryTimes=tryTimes, sleepTimes=sleepTimes)
