...
class binance():
    ...
    def __init__(self, apiKey=None, secret=None, proxies=False, timeout=1, order_timeout=10, verify=False, get_server_time=False, verbose=True):
        self.apiKey = apiKey
        self.secret = secret
            ...
            try:
                ...
                if type(ret) is dict:
                    if ret.get('code') is None:
                        ...
                        print(ret, func_name, '\n交易所返回报错')
                        self.sender.send(ret, func_name, '\n交易所返回报错')
                        # raise ValueError
                else:
                    return ret
            except Exception as e:
