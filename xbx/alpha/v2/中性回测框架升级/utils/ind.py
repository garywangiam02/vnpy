...
def gen_select_df(df, c_rate, select_coin_num):
    # 根据因子对比进行排名
    # 从小到大排序
    df_up = df[(df['filter'] >= 0)]
    df_up['排名1'] = df_up.groupby('candle_begin_time')['因子'].rank(method='first')
    df1 = df_up[(df_up['排名1'] <= select_coin_num)].copy()
    df1['方向'] = 1

    # 从大到小排序
    df_dn = df[(df['filter'] <= 0)]
    df_dn['排名2'] = df_dn.groupby('candle_begin_time')['因子'].rank(method='first', ascending=False)
    df2 = df_dn[(df_dn['排名2'] <= select_coin_num)].copy()
    df2['方向'] = -1
