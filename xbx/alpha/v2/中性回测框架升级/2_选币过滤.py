...
from config import trade_type_list, filter_hour_list
from config import pickle_path, data_path
from config import head_columns

from filters import class_list
...
def cal_one_factor(df, factor_name):
    ...
    """ ******************** 以下是需要修改的代码 ******************** """
    # =====技术指标
    _cls = __import__('filters.%s' % factor_name,  fromlist=('', ))
    for n in filter_hour_list:
        _factor_name = f'{factor_name}_filter_{n}'
        # 计算因子
        getattr(_cls, 'signal')(df, n, _factor_name)
        # 为了跟实盘保持一致 
    ...
    for trade_type in trade_type_list:
        ...
        for symbol, pkl_file_list in _path_merge.items():
            ...
            for df, factor_name in results:
                ...
                # 保存文件头
                # 保存因子
                _factor_path = os.path.join(_symbol_path, f'coin_alpah_filter_{factor_name}.pkl')
