...
all_data = all_data[all_data['candle_begin_time'] <= pd.to_datetime('2021-02-01')]
# =删除某些行数据
all_data = all_data[all_data['volume'] > 0]  # 该周期不交易的币种
all_data.dropna(subset=['下个周期_avg_price'], inplace=True)  # 最后几行数据，下个周期_avg_price为空
print('读取数据完毕!!!\n')
...
def evaluate(arg):
	...
	# ===读取数据
	factor_name = []
	factor_name.extend(head_columns + ['offset', 'filter'])
	factor_name.extend(_factor_columns(factor_list))
	df = all_data[factor_name]

	result = 0
	for offset in range(hold_peroid):
		result += evaluate_offset((df[df['offset'] == offset].copy(), factor_list, offset))
...
_lb   = [0, 0,   0,  0] * len(factors)
_ub   = [1, bhl, 10, len(diff_list)] * len(factors)
_lbin = [1, 1,   1,  1] * len(factors)
_ubin = [1, 1,   1,  1] * len(factors)
...
class alphaFactory(ea.Problem):  # 继承Problem父类
	def __init__(self):
		...
		Dim 	  = len(factors)*4  # 初始化Dim（决策变量维数）
