...
# =====计算资金曲线
# Binance交割合约（usdt本位）资金曲线
def equity_curve_for_Binance_USDT_future_next_open(df, slippage=1 / 1000, c_rate=5 / 10000,
                                                   leverage_rate=3,
    ...
    # 2021-05-31修改
    # 计算下单量 quantity
    df.loc[open_pos_condition, 'quantity'] = initial_cash * leverage_rate / df['open']
    df.loc[open_pos_condition, 'quantity'] = round(df.loc[open_pos_condition, 'quantity'], quantity_precision)

    # 开仓价格：理论开盘价加上相应滑点
    df.loc[open_pos_condition, 'open_pos_price'] = df['open'] * (1 + slippage * df['pos'])
    # 开仓之后剩余的钱，扣除手续费
    df['cash'] = initial_cash - df['open_pos_price'] * df['quantity'] * c_rate  # 即保证金

    # ===开仓之后每根K线结束时
    # 买入之后cash，quantity，open_pos_price不再发生变动
    for _ in ['quantity', 'open_pos_price', 'cash']:
        df[_].fillna(method='ffill', inplace=True)
    df.loc[df['pos'] == 0, ['quantity', 'open_pos_price', 'cash']] = None

    # ===在平仓时
    # 平仓价格
    df.loc[close_pos_condition, 'close_pos_price'] = df['next_open'] * (1 - slippage * df['pos'])
    # 平仓之后剩余的钱，扣除手续费
    df.loc[close_pos_condition, 'close_pos_fee'] = df['close_pos_price'] * df['quantity'] * c_rate

    # ===计算利润
    # 开仓至今持仓盈亏
    df['profit'] = df['quantity'] * (df['close'] - df['open_pos_price']) * df['pos']
    # 平仓时理论额外处理
    df.loc[close_pos_condition, 'profit'] = df['quantity'] * (
            df['close_pos_price'] - df['open_pos_price']) * df['pos']
    # 账户净值
    df['net_value'] = df['cash'] + df['profit']
    ...
    df.loc[df['pos'] == 1, 'price_min'] = df['low']
    df.loc[df['pos'] == -1, 'price_min'] = df['high']
    df['profit_min'] = df['quantity'] * (df['price_min'] - df['open_pos_price']) * df['pos']
    # 账户净值最小值
    df['net_value_min'] = df['cash'] + df['profit_min']
    # 计算保证金率
    df['margin_ratio'] = df['net_value_min'] / (df['quantity'] * df['price_min'])
    # 计算是否爆仓
    df.loc[df['margin_ratio'] <= (min_margin_ratio + c_rate), '是否爆仓'] = 1
    ...
    # =====删除不必要的数据，并存储
    df.drop(['next_open', 'quantity', 'open_pos_price', 'cash', 'close_pos_price', 'close_pos_fee',
             'profit', 'net_value', 'price_min', 'profit_min', 'net_value_min', 'margin_ratio', '是否爆仓'],
            axis=1, inplace=True)
