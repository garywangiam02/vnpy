...
symbol = 'ETH-BTC'
rule_type = '1H'  # 策略K线时间周期

start_date = '2021-01-01'  # 起始日期
end_date = '2021-06-30'  # 终止日期

signal_name = 'adaptboll'  # 策略函数名称
para = [240, 0.05]  # 策略参数，根据实际策略需要更改，可以多参数

data_path = root_path + f'/data/pickle_data/spot/{symbol}_5m.pkl'  # 数据文件的位置
drop_days = 0

subplot_list_list = [['%B'], ['BBW']]  # 在此添加与删除副图指标，注意这是一个list套list的结构，放在同一张副图里的指标需要放在同一个子list里，如果不需要副图就设置成空list。
# subplot_list_list = [['BBW']]
...
# face_value = coin_value_table[symbol.split('_')[0]]  # 一张合约的面值
face_value = get_symbol_face_value(symbol)
# 币安下单量精度，供计算资金曲线函数调用
# quantity_precision = symbol_quantity_precision[symbol.split('_')[0].split('-')[0] + symbol.split('_')[0].split('-')[1]]
...
def echarts(exchange, symbol, rule_type, signal_name, para, df):
    ...
    df.drop(['max_equity_curve'], axis=1, inplace=True) # 删掉max_equity_curve
    ...
    curve_list = ['price_curve', 'equity_curve', ] # 剔除max_equity_curve
    ...
    with open(f'可视化_{title}_echarts.html', 'w', encoding='utf-8') as f:
        f.write(html_str)

    print(f'成功输出可视化文件：可视化_{title}_echarts.html')
...
def get_df_with_signal(df, rule_type, signal_name, para):
    ...
    df = getattr(Signals, signal_name)(df.copy(), para=para)
    df = df[['candle_begin_time', 'open', 'high', 'low', 'close', 'volume', 'median', 'upper', 'lower', 'signal']]
    ...
    df = equity_curve_for_Binance_USDT_future_next_open(df, slippage=slippage, c_rate=c_rate,
                                                        leverage_rate=leverage_rate,
                                                        quantity_precision=quantity_precision,
                                                        min_margin_ratio=min_margin_ratio)
    df.drop(['start_time'], axis=1, inplace=True)

    print(f'策略 {signal_name} 在 Binanace 上的最终收益：', df.iloc[-1]['equity_curve'])
...
if __name__ == '__main__':
    ...
    df_data = pd.read_pickle(data_path)  # 读取数据文件，根据实际文件类型，可改为读csv、pkl类型的数据，可修改
