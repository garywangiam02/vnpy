...
def binance_update_cfuture_account(exchange, symbol_config, symbol_info, mode):
    ...
    if mode == 'u模式':
        ...
        # symbol_info.loc[symbol_info['期现仓差'] <= -symbol_info['合约面值'], '持仓方向_'+mode] = -1
        #修改新增
        symbol_info.loc[symbol_info['合约张数'] < 0, '持仓方向_' + mode] = -1
        symbol_info.loc[symbol_info['合约张数'] > 0, '持仓方向_' + mode] = 1
        # 例外情况  修改1改为0
        symbol_info.loc[symbol_info['合约张数'] == 0, '持仓方向_'+mode] = 0  # 当没有持仓时，持仓均价为0，现货多头为0；期货头寸为0，整体为0。
    ...
    for symbol in symbol_signal.keys():  # {'eth-usdt': '开空', 'eos-usdt': '平多，开空'}
        ...
        if mode == 'u模式':
            if signal_type in ['平空', '平多']:
                ...
                #     symbol, '合约面值']   #修改删除改行代码
                target_future_num = 0  #由*=-1修改为=0,并删除上面的计算公式
