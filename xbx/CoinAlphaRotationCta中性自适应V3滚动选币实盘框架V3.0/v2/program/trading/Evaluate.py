...
def equity_curve_with_long_and_short(df, leverage_rate=3, c_rate=2.0/1000, min_margin_rate=0.15):
    ...
    df['sell_next_open_change'].iloc[-1] = 0
    ...
    # df.loc[open_pos_condition, 'cash'] -= df.loc[open_pos_condition, 'position'] * c_rate  # 减去开仓时的手续费    
    ...
    df.drop(['change', 'buy_at_open_change', 'sell_next_open_change', 'start_time', 'position', 'position_max',
             'position_min', 'profit_min', 'cash', 'cash_min'], axis=1, inplace=True)
...
def sharpe_annual(net_value, periods=252):
    ...
    :param net_value: A pandas Series representing period net value.
    :param periods: Daily (252/365), Hourly (252/365 * 4/24), Minutely(252/365 * 4/24 *60) etc.
    :return:
    '''
    periods = int(periods)
