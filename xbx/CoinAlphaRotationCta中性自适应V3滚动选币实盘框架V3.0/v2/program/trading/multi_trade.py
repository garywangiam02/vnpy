...
class trade():
    ...
    def delete_strategys(self,configs):
        ...
        clear_coins = [ x.split('_')[0] for x in filtered_son]
    ...
    def clear_coin_position(self,clear_coin_name,position_amount):
        """
        币种平指定仓位
        clear_coin_name : 币种
        ...
        param = {"symbol":clear_coin_name,"side": direction, "type": "MARKET","quantity":abs(position_amount)} 
        asyncio.set_event_loop(asyncio.new_event_loop())
        loop = asyncio.get_event_loop() 
    ...
    def clear_strategy_position(self,strategy_name):
        ...
        for son in self.son:
            if(son.name == strategy_name):
                self.clear_coin_position(son.symbol,son.last_amt)
                return  
