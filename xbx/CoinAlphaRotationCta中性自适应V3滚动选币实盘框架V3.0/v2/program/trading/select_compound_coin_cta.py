...
def cal_strategy_config(select_coin):
    ...
    梳理策略配置
    """
    select_coin['final_score'] = select_coin[['c11', 'c12']].sum(axis=1, min_count=1)
    select_coin['param'] = select_coin.apply(lambda x: 11 if x['final_score'] == x['c11'] else 12,axis = 1)
    ...
    select_coin = select_coin[0:selected_strategy_num]
    strategy_config = {}
    select_coin = select_coin[0:selected_strategy_num]
    for i, r in select_coin.iterrows():
        ...
        coin_config['para'] = [r['param']]
        coin_config['data_num'] = 1200  # 策略函数需要多少根k线
        coin_config['time_interval'] = '1h'  # 策略的时间周期
        coin_config['leverage'] = 2  # 策略基础杠杆
        coin_config['weight'] = round(1.5/selected_strategy_num,2)  # 策略分配的资金权重
        config_symbol = coin_config['symbol'] if (coin_config['symbol'])[0:1].isalpha() else "_"+coin_config['symbol'] # 兼容1000SHIBUSDT
...
if __name__ == '__main__':
   ...
   trade_father = trade(apiKey = config['binance']['apiKey'], secret = config['binance']['secret'], config = cta_config, notify_sender = notify_sender, posInfer=False, proxies=False, timeout=1)
   select_coin_apaptbollv3()
   scheduler = BackgroundScheduler()
   #scheduler.add_job(select_coin_apaptbollv3,'cron',month = '1-12',day ='1', hour='8',minute = '45',id = 'select_coin_apaptbollv3')  # 每月1号执行一次
   #scheduler.add_job(select_coin_apaptbollv3,'cron',day_of_week = 'sun', hour='8',minute = '45',id = 'select_coin_apaptbollv3') #每周运行一次
   #scheduler.add_job(reset_coin_config,'cron',day_of_week = 'sun', hour='8',minute = '42',id = 'reset_coin_config') 
   #scheduler.add_job(select_coin_apaptbollv3,'cron', hour='8',minute = '45',id = 'select_coin_apaptbollv3')  #每天执行一次
