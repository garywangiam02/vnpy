...
########################################################################
class StrategyGridTradeFutureV4(CtaProFutureTemplate):
    """期货网格交易策略
    ...
    def __init__(self, cta_engine,
        ...
        self.policy = Future_Grid_Trade_PolicyV4(self)
    ...
    def update_grids(self, vt_symbol):
        ...
        if self.active_long_grid and (self.long_stop_price is None or self.long_stop_price < cur_price):
            ...
            if self.is_chanlun_fit(direction=Direction.LONG, vt_symbol=vt_symbol):
                ...
                if len(plan_long_prices) > 0:
                    ...
                    if last_long_bi_start != self.kline_x.cur_bi.start or (last_long_bi_start == self.kline_x.cur_bi.start and volume_rate > 1):
                        # 获取高度
        ...
        if self.active_short_grid and (self.short_stop_price is None or self.short_stop_price > cur_price):
            ...
            if self.is_chanlun_fit(direction=Direction.SHORT, vt_symbol=vt_symbol):
                ...
                # 找出能满足当前价的空单开仓价
                plan_short_prices = [p for p in short_prices.keys() if p <= cur_price]
                # 找出当前持有仓位的最大开空仓价格
                ...
                if len(plan_short_prices) > 0:
                    ...
                    if last_short_bi_start != self.kline_x.cur_bi.start or (last_short_bi_start == self.kline_x.cur_bi.start and volume_rate > 1):
                        # 止盈数
                        grid_height = grid_plan.get('grid_height', grid_height)
                        # 检查是否存在这个价格的网格
...
class Future_Grid_Trade_PolicyV4(CtaPolicy):
