...
# 示例代码
# 从pytdx下载某一主力合约的1分钟K线，生成本级别和次级别K线，识别出三买三卖信号点，标注在图上
...
class DemoStrategy(FakeStrategy):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 如果之前存在，移除
        if os.path.exists(demo_15_dist):
            self.write_log(f'移除{demo_15_dist}')
            os.remove(demo_15_dist)
    ...
    def on_bar_y(self, *args, **kwargs):
        ...
        # 当前分笔与上一次检查的分笔，开始时间不同，执行检查
        if self.kline_x.cur_bi.start != self.last_check_bi:
            if self.kline_x.cur_datetime.strftime('%Y-%m-%d %H:%M') > '2021-03-08 09:30':
                a = 1
            # 判断三卖
            if self.kline_x.cur_duan.direction == -1:
                result01 = check_zs_3rd(big_kline=self.kline_x,
                                        small_kline=None,
                                        signal_direction=Direction.SHORT)
                if result01:
                    self.last_check_bi = self.kline_x.cur_bi.start
                    # 写入记录
                    append_data(file_name=demo_15_dist,
                                field_names=self.dist_fieldnames,
                                dict_data={
                                    'datetime': datetime.strptime(self.kline_x.cur_bi.start, '%Y-%m-%d %H:%M:%S'),
                                    'symbol': self.symbol,
                                    'volume': 0,
...
ui.show(snapshot_file="",
        d=snapshot,  # 切片数据
        dist_file=demo_15_dist,  # 本地dist csv文件
        dist_include_list=['三卖', '三买'])  # 指定输出的文字内容
