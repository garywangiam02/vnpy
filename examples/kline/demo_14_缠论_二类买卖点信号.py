...
# 示例代码
# 从pytdx下载某一主力合约的1分钟K线，生成本级别和次级别K线，识别出二买二卖信号点，标注在图上
...
class DemoStrategy(FakeStrategy):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 如果之前存在，移除
        if os.path.exists(demo_14_dist):
            self.write_log(f'移除{demo_14_dist}')
            os.remove(demo_14_dist)
...
ui.show(snapshot_file="",
        d=snapshot,  # 切片数据
        dist_file=demo_14_dist,  # 本地dist csv文件
        dist_include_list=['趋势背驰后二卖', '趋势背驰后二买'])  # 指定输出的文字内容
