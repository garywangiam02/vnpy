...
# 示例代码
# 从pytdx下载某一主力合约的1分钟K线，生成本级别和次级别K线，识别出区间套1买一卖信号点，标注在图上
...
class DemoStrategy(FakeStrategy):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 如果之前存在，移除
        if os.path.exists(demo_13_dist):
            self.write_log(f'移除{demo_13_dist}')
            os.remove(demo_13_dist)
    ...
    def on_bar_y(self, *args, **kwargs):
        ...
        if self.kline_x.cur_bi.end != self.last_check_bi:
            ...
            if self.kline_x.cur_duan.direction == 1:
                result01 = check_qjt_1st(big_kline=self.kline_x,
                                        small_kline=self.kline_y,
                                        signal_direction=Direction.SHORT)
                result02 = check_qjt_1st(big_kline=self.kline_x,
                                         small_kline=None,
                                         signal_direction=Direction.SHORT)
                if result01 or result02:
                    self.last_check_bi = self.kline_x.cur_bi.end
                    # 写入记录
                    append_data(file_name=demo_13_dist,
...
ui.show(snapshot_file="",
        d=snapshot,  # 切片数据
        dist_file=demo_13_dist,  # 本地dist csv文件
        dist_include_list=['区间套一卖', '区间套一买'])  # 指定输出的文字内容
