...
# 示例代码
# 从pytdx下载某一合约的1分钟K线，生成本级别和次级别K线，识别出盘整背驰信号点，标注在图上
...
class DemoStrategy(FakeStrategy):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 如果之前存在，移除
        if os.path.exists(demo_10_dist):
            self.write_log(f'移除{demo_10_dist}')
            os.remove(demo_10_dist)
    ...
    def on_bar_y(self, *args, **kwargs):
        ...
        if self.kline_x.cur_bi.end != self.last_check_bi:
            if self.kline_x.cur_datetime.strftime('%Y-%m-%d %H:%M') > '2021-05-18 22:00':
                a = 1
            # 判断盘整背驰
            if self.kline_x.cur_duan.direction == 1:
                # 本级别和次级别一起判断
                result01 = check_pzbc_1st(big_kline=self.kline_x,
                                        small_kline=self.kline_y,
                                        signal_direction=Direction.SHORT)
                # 本级别判断
                result02 = check_pzbc_1st(big_kline=self.kline_x,
                                          small_kline=None,
                                          signal_direction=Direction.SHORT)
                if result01 or result02:
                    self.last_check_bi = self.kline_x.cur_bi.end
                    # 写入记录
                    append_data(file_name=demo_10_dist,
...
ui.show(snapshot_file="",
        d=snapshot,  # 切片数据
        dist_file=demo_10_dist,  # 本地dist csv文件
        dist_include_list=['盘整背驰一卖', '盘整背驰一买'])  # 指定输出的文字内容
